#!/bin/bash
# Start MiniMax-M2.7 on vasp-01:8084 (CPU, 300B MoE / 9B active).
# Experts mmap'd into 256GB system RAM (--no-mmap forces full load).
#
# Binary: llama-server-turboquant (TheTom fork).
# KV cache: q8_0 (NOT turbo4) — empirical: turbo4 is a VRAM optimization,
# costs ~2× latency on CPU-only inference. q8_0 is the best CPU tradeoff.
# Throughput: ~3-4 tok/s output (DDR4 bandwidth-bound).
set -euo pipefail

LLAMA_SERVER="${LLAMA_SERVER:-/usr/local/bin/llama-server-turboquant}"
LOG_PATH="${LOG_PATH:-/tmp/llama-minimax-q8.log}"
QUANT="${QUANT:-UD-Q3_K_M}"
MODEL_FILE="${MODEL_FILE:-/scratch/ai/models/${QUANT}/MiniMax-M2.7-${QUANT}-00001-of-00004.gguf}"
PORT="${PORT:-8084}"

CUDA_ROOT="/opt/nvidia/hpc_sdk/Linux_x86_64/24.11"
export LD_LIBRARY_PATH="${CUDA_ROOT}/cuda/12.6/lib64:${CUDA_ROOT}/cuda/12.6/compat:${CUDA_ROOT}/math_libs/12.6/targets/x86_64-linux/lib:/usr/local/lib:${LD_LIBRARY_PATH:-}"

if [[ ! -x "${LLAMA_SERVER}" ]]; then
  echo "ERROR: llama-server-turboquant not found: ${LLAMA_SERVER}" >&2
  exit 1
fi

# Stop any prior instance on this port; give TCP time to release
existing_pids="$(ps -eo pid=,args= | awk -v port="--port ${PORT}" '$0 ~ "llama-server.*" port "( |$)" { print $1 }')"
if [[ -n "${existing_pids}" ]]; then
  kill ${existing_pids} 2>/dev/null || true
  sleep 5
fi

if [[ ! -f "${MODEL_FILE}" ]]; then
  echo "ERROR: Model file not found: ${MODEL_FILE}" >&2
  exit 1
fi

nohup env LD_LIBRARY_PATH="${LD_LIBRARY_PATH}" numactl --interleave=all \
  "${LLAMA_SERVER}" \
  --model "${MODEL_FILE}" \
  --alias MiniMax-M2.7 \
  --host 0.0.0.0 --port "${PORT}" \
  --ctx-size 32768 --n-gpu-layers 0 \
  --threads 48 --batch-size 512 --ubatch-size 512 \
  --cache-type-k q8_0 --cache-type-v q8_0 \
  --cache-prompt -fa on --no-mmap --cont-batching --metrics --jinja \
  --reasoning off \
  > "${LOG_PATH}" 2>&1 &

echo "Started MiniMax-M2.7 (${QUANT}) PID=$! port=${PORT} binary=${LLAMA_SERVER}"
echo "Expected warm-up: ~2-3 min to load ${MODEL_FILE}"
echo "Expected throughput: ~3-4 tok/s output (CPU-bound MoE routing)"
