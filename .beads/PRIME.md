...
### native-beads-migration-complete
BeadHub (bdh) is no longer used by the swarm. The orchestrator uses native `bd` (v0.62+) directly for all issue tracking and messaging. Scripts default to `scripts/bd-safe.sh` (a thin `bd` wrapper that handles git worktree root resolution); this is controlled by the `SWARM_BEADS_BIN` env var. `bd mail` is the native messaging layer replacing BeadHub `:aweb`. Identity is set via `BD_ACTOR` env var. `bdh` remains available on the local Mac and ai-proxy for human convenience (adds `:status`, `:policy`, `:aweb` coordination commands) but is never called by swarm agents or scripts.
